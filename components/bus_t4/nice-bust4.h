...
static const float CLOSED_POSITION_THRESHOLD = 0.007f;
...
class NiceBusT4 : public cover::Cover, public Component, public uart::UARTDevice {
 public:
  ...
  // Drive settings
  bool autocls_flag{false};    // Auto-closing - L1
  bool photocls_flag{false};   // Close after photo - L2
  bool alwayscls_flag{false};  // Always close - L3
  ...
  void send_inf_cmd(std::string to_addr, std::string whose, std::string command, std::string type_command, std::string next_data,
  ...
  // ⬅️ importante: público en ESPHome moderno
 ...
 protected:
  ...
  cover::CoverOperation last_published_op{cover::COVER_OPERATION_IDLE};
  float last_published_pos{-1.0f};

  void publish_state_if_changed();

  uint8_t position_hook_type{IGNORE};
  uint16_t position_hook_value{0};
  ...
  uint8_t addr_to[2] = {0x00, 0xff};
  uint8_t addr_oxi[2] = {0x00, 0x0a};
  ...
  std::vector<uint8_t> gen_inf_cmd(uint8_t whose, uint8_t inf_cmd, uint8_t run_cmd) {
    return gen_inf_cmd(this->addr_to[0], this->addr_to[1], whose, inf_cmd, run_cmd, 0x00, {0x00}, 0);
  }
  std::vector<uint8_t> gen_inf_cmd(uint8_t whose, uint8_t inf_cmd, uint8_t run_cmd, uint8_t next_data, std::vector<uint8_t> data) {
    return gen_inf_cmd(this->addr_to[0], this->addr_to[1], whose, inf_cmd, run_cmd, next_data, data, data.size());
  }
  std::vector<uint8_t> gen_inf_cmd(uint8_t to_addr1, uint8_t to_addr2, uint8_t whose, uint8_t inf_cmd, uint8_t run_cmd, uint8_t next_data) {
